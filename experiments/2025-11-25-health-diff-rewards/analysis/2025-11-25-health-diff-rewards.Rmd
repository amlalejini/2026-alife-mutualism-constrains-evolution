# 2025-11-25 - Health mode differential task rewards
...
```{r}
host_max_task_complexity_sep_extinctions_plt <- host_max_task_complexity_data %>%
  ggplot(
    aes(x = sym_con_extinction, fill = sym_con_extinction, y = max_task_complexity)
  ) +
  geom_point(
    mapping=aes(color = sym_con_extinction),
    position = position_jitter(width = .15),
    size = .5,
    alpha = 0.8
  ) +
...
ggsave(
  file = paste(plot_dir, "host_max_task_complexity_sep_extinctions_plt.pdf", sep = "/"),
  plot = host_max_task_complexity_sep_extinctions_plt,
  width = 10,
  height = 10
)
```
...
# sig_kw_groups <- filter(kw_test, p < 0.05)$interaction_multiplier
...
pairwise.wilcox.test(
  x = stats_data$max_task_complexity,
  g = stats_data$sym_condition,
...
```{r}
TransmissionRates_attempts_horiztrans_zoomed_plt <- time_data %>%
  filter(
    update %% 500 == 0 & interaction_multiplier == "4" & sym_condition != "parasite"
  ) %>%
  ggplot(
    aes(
      x = update,
...
ggsave(
  file = paste(plot_dir, "TransmissionRates_attempts_horiztrans_zoomed_plt.pdf", sep = "/"),
  plot = TransmissionRates_attempts_horiztrans_zoomed_plt,
  width = 15,
  height = 15
)
```
...
```{r}
v <- color("vibrant")(7)
color_pal <- c(v[["blue"]], v[["orange"]], v[["magenta"]])
```
...
#   file = paste(
#     plot_dir,
#     "manuscript_host_parent_entropy_task_sets_plt.pdf",
#     sep = "/"
#   ),
#   plot = manuscript_host_parent_entropy_task_sets_plt,
#   width = 6,
#   height = 4
# )
```
...
save_plot(
  filename = paste(
    plot_dir,
    "fig-diff-rewards-host-complexity-smaller.pdf",
    sep = "/"
  ),
  plot = plt_grid_smaller,
  base_width = 4,
  base_height = 4,
  dpi = 600
)
